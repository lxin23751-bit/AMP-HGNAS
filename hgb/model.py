...
class LMSPS(nn.Module):
    def __init__(self, nfeat, hidden, nclass, feat_keys, label_feat_keys, tgt_type,
        ...
        for k, v in data_size.items(): #为每个元路径创建一个嵌入,映射到同一空间
        ...
        self.layer_final = nn.Linear(num_channels * hidden, hidden)  #将self.layers的输出进一步变换到一个更小的维度空间
    ...
    def forward(self, batch, feature_dict, label_dict={}, mask=None):
        if isinstance(feature_dict[self.tgt_type], torch.Tensor):  #特征嵌入
        ...
        embedding = self.dropout(self.prelu(out))  # 这是分类前的嵌入
        
        out = self.lr_output(embedding)

        return out
    
    def forward_embedding(self, batch, feature_dict, label_dict={}, mask=None):
        """提取节点嵌入（分类层之前）"""
        ...
        embedding = self.dropout(self.prelu(out))
        return embedding
