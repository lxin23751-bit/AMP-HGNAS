...
# 确保可视化函数可用
from utils import plot_embedding_visualization, HAS_UMAP
...
def main(args):
    ...
    g, adjs, init_labels, num_classes, dl, train_nid, val_nid, test_nid, test_nid_full \
        = load_dataset(args)
    print(archs[args.arch][0])
    print("Dataset Info:")

    node_type_counts = {}
    for ntype in g.ntypes:
        node_type_counts[ntype] = g.number_of_nodes(ntype)

    # 获取边类别信息
    edge_type_counts = {}
    for etype in g.etypes:
        edge_type_counts[etype] = g.number_of_edges(etype)
    ...
    # 打印目标节点数
    print(f"Target nodes: {len(test_nid_full)}")
    #archs[args.arch][0] = [x for i, x in enumerate(archs[args.arch][0]) if random.random() > 0.5]
    for k in adjs.keys():
    ...
    print("feats.keys",list(feats.keys()))
    ...
    dataset_label = f'{args.dataset}_seed{args.seed}'
    
    # 提取节点嵌入用于可视化
    print("Extracting node embeddings for visualization...")
    print(f"trainval_point={trainval_point}, valtest_point={valtest_point}, total_num_nodes={total_num_nodes}")
    model.load_state_dict(torch.load(f'{checkpt_file}.pkl', map_location='cpu'), strict=True)
    model = model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch, batch_feats, batch_labels_feats, batch_mask in eval_loader:
            ...
            # 使用forward_embedding提取嵌入
            embedding = model.forward_embedding(batch_gpu, batch_feats, batch_labels_feats, batch_mask)
            
            # 收集所有嵌入和对应的节点索引
            batch_cpu = batch.cpu()
            batch_labels = labels[batch_cpu]
            
            all_embeddings.append(embedding.cpu())
            all_labels_list.append(batch_labels)
            all_node_indices.append(batch_cpu)
    
    # 合并所有嵌入
    if all_embeddings:
        all_embeddings_tensor = torch.cat(all_embeddings, dim=0)
        all_labels_tensor = torch.cat(all_labels_list, dim=0)
        all_indices = torch.cat(all_node_indices, dim=0)
        
        print(f"Total embeddings extracted: {all_embeddings_tensor.shape[0]}")
        
        # 根据节点索引分离测试集和验证集的嵌入
        test_mask = (all_indices >= valtest_point) & (all_indices < total_num_nodes)
        val_mask = (all_indices >= trainval_point) & (all_indices < valtest_point)
        
        test_embeddings = all_embeddings_tensor[test_mask]
        test_emb_labels = all_labels_tensor[test_mask]
        val_embeddings = all_embeddings_tensor[val_mask]
        val_emb_labels = all_labels_tensor[val_mask]
        
        print(f"Test embeddings: {test_embeddings.shape[0]}, Val embeddings: {val_embeddings.shape[0]}")
    else:
        test_embeddings = None
        test_emb_labels = None
        val_embeddings = None
        val_emb_labels = None
    ...
    if test_embeddings is not None and len(test_embeddings) > 0:
        print(f"Generating test set embedding visualization (t-SNE)...")
        try:
            tsne_path = os.path.join(vis_dir, f'embedding_tsne_{dataset_label}_test.png')
            plot_embedding_visualization(test_embeddings, test_emb_labels, tsne_path, dataset_name=f'{dataset_label}_test', method='tsne')
        except Exception as e:
            print(f"Error drawing test TSNE embedding: {e}")
            import traceback
            traceback.print_exc()
        
        if HAS_UMAP:
            try:
                print(f"Generating test set embedding visualization (UMAP)...")
                umap_path = os.path.join(vis_dir, f'embedding_umap_{dataset_label}_test.png')
                plot_embedding_visualization(test_embeddings, test_emb_labels, umap_path, dataset_name=f'{dataset_label}_test', method='umap')
            except Exception as e:
                print(f"Error drawing test UMAP embedding: {e}")
    
    if val_embeddings is not None and len(val_embeddings) > 0:
        print(f"Generating val set embedding visualization (t-SNE)...")
        try:
            tsne_path = os.path.join(vis_dir, f'embedding_tsne_{dataset_label}_val.png')
            plot_embedding_visualization(val_embeddings, val_emb_labels, tsne_path, dataset_name=f'{dataset_label}_val', method='tsne')
        except Exception as e:
            print(f"Error drawing val TSNE embedding: {e}")
            import traceback
            traceback.print_exc()
        
        if HAS_UMAP:
            try:
                print(f"Generating val set embedding visualization (UMAP)...")
                umap_path = os.path.join(vis_dir, f'embedding_umap_{dataset_label}_val.png')
                plot_embedding_visualization(val_embeddings, val_emb_labels, umap_path, dataset_name=f'{dataset_label}_val', method='umap')
            except Exception as e:
                print(f"Error drawing val UMAP embedding: {e}")

    # 测试集可视化
    test_true = labels[:total_num_nodes]
    test_pred = preds[:total_num_nodes].squeeze()
    visualize_results(
        test_true, test_pred, num_classes, vis_dir,
        dataset_name=f'{dataset_label}_test',
        train_accs=train_accs_history if len(train_accs_history) > 0 else None,
        val_accs=val_accs_history if len(val_accs_history) > 0 else None,
        test_accs=test_accs_history if len(test_accs_history) > 0 else None,
        embeddings=test_embeddings if test_embeddings is not None and len(test_embeddings) > 0 else None,
        embedding_labels=test_emb_labels if test_emb_labels is not None and len(test_emb_labels) > 0 else None
    )

    # 验证集可视化
    val_true = labels[:valtest_point]
    val_pred = preds[:valtest_point].squeeze()
    visualize_results(
        val_true, val_pred, num_classes, vis_dir,
        dataset_name=f'{dataset_label}_val',
        embeddings=val_embeddings if val_embeddings is not None and len(val_embeddings) > 0 else None,
        embedding_labels=val_emb_labels if val_emb_labels is not None and len(val_emb_labels) > 0 else None
    )
